...
import { cancelRequestAction, type FormState } from "@/lib/compensation-actions";

const EMPTY: FormState = { ok: false };

export default function CompRequestCancel({ requestId }: { requestId: string }) {
  const [state, formAction, pending] = useActionState(cancelRequestAction, EMPTY);
  ...
  return (
    <form action={formAction} style={{ display: "inline-flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
      ...
      <button type="submit" className="btn" style={{ color: "var(--red)", borderColor: "var(--red)" }} disabled={pending}>
