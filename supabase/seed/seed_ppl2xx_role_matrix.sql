-- =============================================================================
-- seed_ppl2xx_role_matrix.sql  (v0.61.0)
-- Maps the PPL operator track P2 (PPL-201..204) to the People-Ops Officer job
-- profile as REQUIRED, so the newly published modules resolve as assigned work
-- (not orphaned in the library). PPL-2xx were absent from the role matrix; this
-- closes that gap with the same JOB_PROFILE/REQUIRED pattern used elsewhere.
...
-- PPL operator track P2: REQUIRED for the People-Ops Officer.
...
-- People-Ops Officer across PPL-201..204.
DO $guard$
DECLARE n int;
BEGIN
  SELECT count(*) INTO n
  ...
  IF n <> 4 THEN RAISE EXCEPTION 'Guard: expected 4 PPL P2 People-Ops rules (got %)', n; END IF;
END
$guard$;
